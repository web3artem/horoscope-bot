...
async def schedule_morning():
    ids = await User.select('user_id').where(User.receive_day_period == 'morning').gino.all()
    users_id = [i[0] for i in ids]
    for user_id in users_id:
        try:
            await funcs.prepare_data(user_id)
        except BotBlocked:
            logger.info(f'Пользователь {user_id} заблокировал бота. Удаляем из БД')
...
async def schedule_tomorrow():
    ids = await User.select('user_id').where(User.receive_day_period == 'tomorrow').gino.all()
    users_id = [i[0] for i in ids]
    for user_id in users_id:
        try:
            await funcs.prepare_data(user_id)
